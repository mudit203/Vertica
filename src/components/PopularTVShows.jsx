"use client"

import { useState, useEffect } from "react";
import axios from "axios";
import TVShowCarousel from './TVShowCarousel';

const PopularTVShows = () => {
  const [theme, setTheme] = useState('dark');
  const [popularTVContent, setPopularTVContent] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  // Listen for theme changes
  useEffect(() => {
    const handleThemeChange = (event) => {
      setTheme(event.detail);
    };

    // Get initial theme
    const savedTheme = localStorage.getItem('theme') || 'dark';
    setTheme(savedTheme);

    // Listen for theme changes
    window.addEventListener('themeChange', handleThemeChange);

    return () => {
      window.removeEventListener('themeChange', handleThemeChange);
    };
  }, []);

  // Fetch popular TV shows from TMDB API
  useEffect(() => {
    const fetchPopularTVShows = async () => {
      try {
        setLoading(true);
        const response = await axios.get(
          `https://api.themoviedb.org/3/tv/popular?api_key=${process.env.NEXT_PUBLIC_TMDB_API_KEY}&language=en-US&page=1`
        );
        
        // Map API response to match TVShowCard props
        const mappedTVShows = response.data.results.map((show) => ({
          id: show.id.toString(),
          title: show.name,
          poster: `https://image.tmdb.org/t/p/w500${show.poster_path}`,
          year: new Date(show.first_air_date).getFullYear().toString(),
          rating: parseFloat(show.vote_average.toFixed(1)),
          genre: "TV Show",
          description: show.overview,
          seasons: show.number_of_seasons || 1,
          status: show.status || "ongoing"
        }));
        
        setPopularTVContent(mappedTVShows);
        setError(null);
      } catch (err) {
        console.error('Error fetching popular TV shows:', err);
        setError('Failed to load popular TV shows');
      } finally {
        setLoading(false);
      }
    };

    fetchPopularTVShows();
  }, []);


  const handleEpisodeViewerOpen = (show) => {
    console.log("Opening episode viewer for:", show.title);
  };

  if (loading) {
    return (
      <section className={`py-16 transition-colors duration-1000 ${
        theme === 'dark' ? 'bg-black' : 'bg-gray-50'
      }`}>
        <div className="max-w-7xl mx-auto">
          <div className="mb-8 px-8">
            <h2 className={`text-4xl font-bold mb-4 transition-colors duration-1000 ${
              theme === 'dark' ? 'text-white' : 'text-gray-900'
            }`}>Popular TV Shows</h2>
            <p className={`text-lg transition-colors duration-1000 ${
              theme === 'dark' ? 'text-gray-300' : 'text-gray-600'
            }`}>
              Loading popular TV shows...
            </p>
          </div>
        </div>
      </section>
    );
  }

  if (error) {
    return (
      <section className={`py-16 transition-colors duration-1000 ${
        theme === 'dark' ? 'bg-black' : 'bg-gray-50'
      }`}>
        <div className="max-w-7xl mx-auto">
          <div className="mb-8 px-8">
            <h2 className={`text-4xl font-bold mb-4 transition-colors duration-1000 ${
              theme === 'dark' ? 'text-white' : 'text-gray-900'
            }`}>Popular TV Shows</h2>
            <p className={`text-lg transition-colors duration-1000 ${
              theme === 'dark' ? 'text-red-400' : 'text-red-600'
            }`}>
              {error}
            </p>
          </div>
        </div>
      </section>
    );
  }

  return (
    <section id="series" className="relative z-10 py-16 transition-colors duration-1000">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8 px-8">
          <h2 className={`text-4xl font-bold mb-4 transition-colors duration-1000 ${
            theme === 'dark' ? 'text-white' : 'text-gray-900'
          }`}>Popular TV Shows</h2>
          <p className={`text-lg transition-colors duration-1000 ${
            theme === 'dark' ? 'text-gray-300' : 'text-gray-600'
          }`}>
            Most watched TV shows right now
          </p>
        </div>
        
        <TVShowCarousel
          title=""
          shows={popularTVContent}
          onShowClick={handleEpisodeViewerOpen}
        />
      </div>
    </section>
  );
};

export default PopularTVShows;
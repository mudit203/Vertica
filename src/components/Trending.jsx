...
const Trending = () => {
  ...
  useEffect(() => {
    const fetchTrendingMovies = async () => {
      try {
        ...
        setTrendingContent(mappedMovies);
        setError(null);
      } catch (err) {
  ...
  return (
    <section className="relative z-10 py-16 transition-colors duration-1000">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8 px-8">
          <h2 className={`text-4xl font-bold mb-4 transition-colors duration-1000 ${
            theme === 'dark' ? 'text-white' : 'text-gray-900'
          ...
          <p className={`text-lg transition-colors duration-1000 ${
            ...
            Discover the most popular content everyone's watching
          </p>
        </div>
        
        <TVShowCarousel
          title=""
          shows={trendingContent}
